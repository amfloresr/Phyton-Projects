...
BOARDWIDTH=10 #Números de columnas.
...
#Figuras que saldrán en la ventana.
DONUT="dona"
SQUARE="cuadrado"
DIAMOND="diamante"
LINES="lineas"
...
assert len(ALLCOLORS)*len(ALLSHAPES)*2>=BOARDWIDTH*BOARDHEIGHT, "El tablero es demasiado grande para la cantidad de formas y colores definidos."
...
def main():
	...
	while True: #Bucle principal del juego
			...
			if not revealedBoxes[boxx][boxy] and mouseClicked:
				...
				if firstSelection==None: #La caja actual fué la primera.
					...
					if icon1shape!=icon2shape or icon1color!=icon2color:
						...
						#Muestreo del tablero completamente no revelado por un segundo.
						drawBoard(mainBoard,revealedBoxes)
		...
		#Vuelve a dibujar la pantalla y espere un tic del reloj.
		pygame.display.update()
		FPSCLOCK.tick(FPS)
...
def getRandomizedBoard():
	...
	random.shuffle(icons) #Aleatoriza el orden de los iconos.
	...
	for x in range(BOARDWIDTH):
		...
		for y in range(BOARDHEIGHT):
			...
			del icons[0] #Elimina los iconos que les asignamos.
		board.append(column)
	return board

def splitIntoGroupsOf(groupSize,theList):
	#Divide una lista en lista de listas.
	result=[]
...
def drawIcon(shape,color,boxx,boxy):
	...
	left,top=leftTopCoordsOfBox(boxx,boxy) #obtiene las coordenadas de los píxeles de los cables de la placa.
	#Se procede al dibujo de formas.
	if shape==DONUT:
...
def getShapeAndColor(board,boxx,boxy):
	#valor del color para el punto x, y se almacena en el tablero.
...
def gameWonAnimation(board):
	...
	for i in range (13):
		color1,color2=color2,color1 #Aquí se intercambian colores
